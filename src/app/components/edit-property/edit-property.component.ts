...
export class EditPropertyComponent implements OnInit,AfterViewInit,OnDestroy {
  ...
  dropzoneInit() {
    ...
    this.myDropzone = new Dropzone("form.dropzone", {
      ...
      addRemoveLinks: true,
      dictDefaultMessage: "<i class='sl sl-icon-plus'></i> اضغط هنا لتحميل صور العقار",
      init: function() {
  ...
  chosenPlugin() {
    var config = {
      ...
      '.chosen-select-no-single-ar' : {disable_search_threshold:100, width:"100%", placeholder_text_single: "اختر"},
  ...
  validationStyles() {
    $('select.ng-valid + .chosen-container>a').css('border-color', '#5f9025');
    $('select.ng-invalid + .chosen-container>a').css('border-color', '#de5959');
  }
  ...
  chosenAgeType() {
    ...
    // attach a change event
    $('.api-updated-agetype').on('change', (ev, params) => {
      self.submitForm.controls.ageType.setValue(params.selected);
      setTimeout(() => self.validationStyles(), 110);
    })
  }
  chosenServiceType() {
  ...
  chosenQualityType() {
    ...
    // attach a change event
    $('.api-updated-qualitytype').on('change', (ev, params) => {
      self.submitForm.controls.qualityType.setValue(params.selected);
      setTimeout(() => self.validationStyles(), 110);
    })
  }
  chosenCurrency(currency) {
    ...
    // attach a change event
    $('.api-updated-currency').on('change', (ev, params) => {
      self.submitForm.controls.currency.setValue(params.selected);
      setTimeout(() => self.validationStyles(), 110);
    })
  }
  chosenPriceType(priceType) {
    ...
    // attach a change event
    $('.api-updated-priceType').on('change', (ev, params) => {
      self.submitForm.controls.priceType.setValue(params.selected);
      setTimeout(() => self.validationStyles(), 110);
    })
  }
  chosenMeasureType(measureType) {
    ...
    // attach a change event
    $('.api-updated-measuretype').on('change', (ev, params) => {
      self.submitForm.controls.landSizeMeasureType.setValue(params.selected);
      setTimeout(() => self.validationStyles(), 110);
    })
  }
  chosenChange() {
    ...
    $('.chosen-bathrooms').on('change', (ev, params) => {
      self.submitForm.controls.bathrooms.setValue(params.selected);
      setTimeout(() => self.validationStyles(), 110);
    })
    $('.chosen-bedrooms').on('change', (ev, params) => {
      self.submitForm.controls.bedrooms.setValue(params.selected);
      setTimeout(() => self.validationStyles(), 110);
    })
  }
  commonEnable() {
